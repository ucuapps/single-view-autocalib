classdef p2x2c4 < handle & WRAP.DivisionSolver
    ...
    methods(Static)
        function M = solve(x, c, arcs)
            ...
            %%%% Rectification from 2PC+(2+2)CA
            x_rgns = reshape(x, 18, []);
            for k=1:size(idx_configs,1) 
                idx = reshape((idx_configs(k,:)-1).*3 + [1;2;3],[],1);
                tic
                [q1, ~, u1, v1, w1] = NP_MC_to_qlu(x_rgns(idx,:), c([4:7 11:14],:));
                soltime1 = toc;
            ...
            %%%% Auto-calibration from 2PC+(2+2)CA
            for k=1:size(idx_configs,1) 
                idx = reshape((idx_configs(k,:)-1).*3 + [1;2;3],[],1);
                tic
                [q1, u1, v1, w1] = NP_MC_to_qfu(x_rgns(idx,:), c([4:7 11:14],:));
                soltime1 = toc;
                soltime1 = soltime1*ones(size(q1));
                uvw1 = [u1;v1;w1];
                valid = WRAP.DivisionSolver.valid_q(q1);
            ...
            for k2=1:2
                for k=1:size(idx_configs2,2)
                    ...
                    tic
                    [q1, ~, u1, v1, w1] = NP_MC_to_qlu([x_rgns(idx1,:) x_rgns(idx2,:)], c([4:7 11:14],k2));
                    soltime1 = toc;
                    soltime1 = soltime1*ones(size(q1));
                    uvw1 = [u1;v1;w1];
                    valid = WRAP.DivisionSolver.valid_q(q1);
                    ...
                    uvw = [uvw uvw1];
                    soltime = [soltime soltime1];
                    consistency = [consistency eval_min_hybrid(q1,uvw1,x,arcs((k2-1)*2+1:k2*2),reshape(c(:,k2),7,[]))];
                end
            end

function [ps,ix] = dp_simplify(p,tol)

    % Recursive Douglas-Peucker Polyline Simplification, Simplify
    %
    % [ps,ix] = dp_simplify(p,tol)
    %
    % dp_simplify uses the recursive Douglas-Peucker line simplification 
    ...
    %
    % In case of nans (missing vertex coordinates) dp_simplify assumes that 
    % nans separate polylines. As such, dp_simplify treats each line
    % separately.
    %
    ...
    elseif Inanp;
        ...
        % now call dp_simplify again inside cellfun. 
        if nargout == 2;
            [ps,ix]   = cellfun(@(x) dp_simplify(x,tol),c,'uniformoutput',false);
            ix        = cellfun(@(x,six) x+six-1,ix,num2cell(sIX),'uniformoutput',false);
        else
            ps   = cellfun(@(x) dp_simplify(x,tol),c,'uniformoutput',false);
        end
